...
# База одна на три площадки — та же, что у Telegram-бота (intercity_db).
# PG_DSN2 оставлена запасным входом: если хостинг вдруг перестанет
# обновлять PG_DSN в контейнере, можно задать значение через неё.
PG_DSN = os.getenv("PG_DSN2") or os.getenv("PG_DSN")

# Все три площадки живут в одной таблице drivers с общим ключом user_id.
# ВК-id короткие, телеграмные длиннее — рано или поздно совпали бы
# и один водитель перезаписал другого. Поэтому разносим по диапазонам:
#   Telegram — как есть, MAX — +10 трлн, ВК — +20 трлн.
MAX_ID_OFFSET = 10_000_000_000_000
VK_ID_OFFSET  = 20_000_000_000_000


def vk_uid(raw_id) -> int:
    """id пользователя ВК → общий id."""
    return VK_ID_OFFSET + int(raw_id)


def vk_raw(uid) -> int:
    """Общий id → настоящий id во ВКонтакте."""
    return int(uid) - VK_ID_OFFSET


def uid_platform(uid) -> str:
    """Какой площадке принадлежит id — одним сравнением, без базы."""
    try:
        n = int(uid)
    ...
    if n >= MAX_ID_OFFSET:
        return "max"
    return "tg"
# В переменной окружения админы заданы обычными ВК-id.
# Приводим к общему виду сразу — иначе отправка сочтёт их телеграмными
# и сложит сообщения в очередь, откуда их никто не заберёт.
ADMIN_IDS = [vk_uid(int(x)) for x in os.getenv("ADMIN_IDS", "").split(",") if x]
...
TARIFFS_RF = {
    "standard":    {"label": "🚗 Стандарт",  "price": 29,   "price_seat": 11},
    "comfort":     {"label": "🚙 Комфорт",   "price": 35,   "price_seat": 15},
...
TARIFFS_NT = {
    "standard":    {"label": "🚗 Стандарт",  "price": 85,   "price_seat": 40},
    "comfort":     {"label": "🚙 Комфорт",   "price": 95,   "price_seat": 45},
    "comfort_plus":{"label": "✨ Комфорт+",  "price": 100,  "price_seat": 50},
    "minivan":     {"label": "🚐 Минивэн",   "price": 115,  "price_seat": 50},
    "business":    {"label": "💼 Бизнес",    "price": 130,  "price_seat": 65},
}
TARIFFS_CIS = {
...
async def fsm_cleanup_task():
    ...
    while True:
        ...
        for uid in stale:
            ...
            try:
                await bot.state_dispenser.delete(vk_raw(uid))
            except KeyError:
                pass
            except Exception:
                pass
...
async def init_pg():
    ...
    # Спрашиваем у самого сервера, к какой базе мы подключились и что в ней лежит.
    async with _pg_pool.acquire() as conn:
        dbname = await conn.fetchval("SELECT current_database()")
        cnt = await conn.fetchval("SELECT COUNT(*) FROM orders")
        mx  = await conn.fetchval("SELECT COALESCE(MAX(id),0) FROM orders")
        src = "PG_DSN2" if os.getenv("PG_DSN2") else "PG_DSN"
        log.info(f"🗄️ База: {dbname} | заказов: {cnt} | последний id: {mx} | из {src}")
    async with _pg_pool.acquire() as conn:
        # Общая очередь сообщений между площадками. Создаёт тот бот,
        # кто стартовал первым; у второго команда просто ничего не делает.
        await conn.execute("""
        ...
        """)
        await conn.execute(
            "CREATE INDEX IF NOT EXISTS outbox_pending ON outbox(platform, delivered) "
            "WHERE delivered = FALSE")
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS order_announced(
        ...
        """)
        await conn.execute(
            "ALTER TABLE drivers ADD COLUMN IF NOT EXISTS platform TEXT DEFAULT 'vk'")
    async with _pg_pool.acquire() as conn:
        await conn.execute("""
...
class FSMRestoreMiddleware(BaseMiddleware[Message]):
    ...
    async def pre(self):
        ...
        if self.event.text in RESET_COMMANDS:
            return
        try:
            current_state = await bot.state_dispenser.get(vk_raw(uid))
        except KeyError:
            current_state = None
        if current_state is None:
            ...
            if saved_state_str is not None:
                ...
                if state_obj:
                    await bot.state_dispenser.set(vk_raw(uid), state_obj)
...
async def safe_send(user_id, text, keyboard=None):
    """Единственная точка отправки. Своим пишем напрямую, а водителям
    из Telegram и MAX мы писать не умеем — кладём в общую очередь,
    оттуда заберёт соответствующий бот. Если он лежит — доставит позже."""
    plat = uid_platform(user_id)
    if plat != "vk":
        await outbox_put(user_id, plat, text)
        return
    try:
        await bot.api.messages.send(
            user_id=vk_raw(user_id),
            message=text,
            keyboard=keyboard,
...
async def outbox_put(user_id, platform, text):
    """Кладёт сообщение в очередь для чужой площадки."""
    try:
        async with _pg_pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO outbox(user_id, platform, text) VALUES($1,$2,$3)",
                int(user_id), platform, text)
    except Exception as e:
        log.error(f"Очередь исходящих недоступна ({platform} {user_id}): {e}")


async def outbox_worker():
    """Забирает из очереди то, что адресовано ВК, и доставляет."""
    await asyncio.sleep(15)
    while True:
        try:
            async with _pg_pool.acquire() as conn:
                rows = await conn.fetch(
                    "SELECT id, user_id, text FROM outbox "
                    "WHERE platform='vk' AND delivered=FALSE AND attempts < 5 "
                    "ORDER BY id LIMIT 50")
                for r in rows:
                    try:
                        await bot.api.messages.send(
                            user_id=vk_raw(r["user_id"]),
                            message=r["text"],
                            random_id=random.randint(1, 2**31 - 1))
                        await conn.execute(
                            "UPDATE outbox SET delivered=TRUE WHERE id=$1", r["id"])
                    except Exception as e:
                        await conn.execute(
                            "UPDATE outbox SET attempts=attempts+1 WHERE id=$1", r["id"])
                        log.error(f"Очередь: не доставлено {r['user_id']}: {e}")
        except asyncio.CancelledError:
            raise
        except Exception as e:
            log.error(f"outbox_worker: {e}")
        await asyncio.sleep(10)


async def announce_worker():
    """Заказы создаются на любой площадке и ложатся в общую таблицу.
    Эта задача находит те, о которых ещё не объявляли своим водителям."""
        ...
        try:
            async with _pg_pool.acquire() as conn:
                rows = await conn.fetch("""
                    ...
                    WHERE o.status = 'open'
                      AND NOT EXISTS (SELECT 1 FROM order_announced a
                                      WHERE a.order_id = o.id AND a.platform = 'vk')
                    ORDER BY o.id DESC LIMIT 20
                """)
            for r in rows:
                oid = r["id"]
                try:
                    await notify_drivers_about_order(oid)
                finally:
                    async with _pg_pool.acquire() as conn:
                        await conn.execute(
                            "INSERT INTO order_announced(order_id, platform) VALUES($1,'vk') "
                            "ON CONFLICT DO NOTHING", oid)
        except asyncio.CancelledError:
            raise
        except Exception as e:
            log.error(f"announce_worker: {e}")
...
async def notify_drivers_about_order(order_id, exclude_drivers=None):
    if exclude_drivers is None:
        exclude_drivers = []
    # Отметка в базе: иначе после перезапуска задача разбора
    # сочла бы заказ новым и разослала его второй раз.
    try:
        async with _pg_pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO order_announced(order_id, platform) VALUES($1,'vk') "
        ...
        log.error(f"Отметка о рассылке #{order_id}: {e}")
    try:
        async with _pg_pool.acquire() as conn:
            drivers = await conn.fetch("""
                SELECT d.user_id, d.car_class FROM drivers d
                JOIN subscriptions s ON d.user_id = s.user_id
                WHERE d.docs_verified::int = 1 AND s.expires_date::date >= NOW()::date
                  AND d.platform = 'vk'
            """)
...
async def start_handler(message: Message):
    uid = vk_uid(message.from_id)
    if await check_blacklist(vk_uid(message.from_id)):
...
async def start_order(message: Message):
    ...
    if await check_blacklist(vk_uid(message.from_id)):
        ...
        return
    uid = vk_uid(message.from_id)
    await bot.state_dispenser.set(vk_raw(uid), OrderStates.from_city)
    fsm_data[uid] = {}
...
async def order_from_city(message: Message):
    ...
    await bot.state_dispenser.set(vk_raw(uid), OrderStates.to_city)
...
async def order_to_city(message: Message):
    ...
    data["to_city"] = city
    fsm_data[uid] = data
    await bot.state_dispenser.set(vk_raw(uid), OrderStates.trip_date)
...
async def order_trip_date(message: Message):
    ...
    fsm_data[uid] = data
    await bot.state_dispenser.set(vk_raw(uid), OrderStates.trip_time)
...
async def order_trip_time(message: Message):
    ...
    fsm_data[uid] = data
    await bot.state_dispenser.set(vk_raw(uid), OrderStates.passengers)
...
async def order_passengers(message: Message):
    ...
    if passengers >= 5:
        ...
        hint = ""
    kb.add(Text("❌ Отменить"), color=KeyboardButtonColor.NEGATIVE)
    await bot.state_dispenser.set(vk_raw(uid), OrderStates.car_class)
...
async def order_car_class(message: Message):
    ...
    fsm_data[uid] = data
    await bot.state_dispenser.set(vk_raw(uid), OrderStates.wishes)
    ...
    async with _pg_pool.acquire() as conn:
        order = await conn.fetchrow(
            """INSERT INTO orders(
                ...
                passengers, car_class, wishes, distance_km, price, status
...
async def driver_menu_handler(message: Message):
    ...
    if not is_registered:
        await bot.state_dispenser.set(vk_raw(uid), DriverStates.phone)
        fsm_data[uid] = {}
...
async def driver_phone(message: Message):
    ...
    await bot.state_dispenser.set(vk_raw(uid), DriverStates.car_model)
...
async def driver_car_model(message: Message):
    ...
    fsm_data[uid] = data
    await bot.state_dispenser.set(vk_raw(uid), DriverStates.car_year)
...
async def driver_car_year(message: Message):
    ...
    data['car_year'] = year
    fsm_data[uid] = data
    await bot.state_dispenser.set(vk_raw(uid), DriverStates.car_number)
...
async def driver_car_number(message: Message):
    ...
    fsm_data[uid] = data
    await bot.state_dispenser.set(vk_raw(uid), DriverStates.car_class)
...
async def driver_car_class(message: Message):
    ...
    async with _pg_pool.acquire() as conn:
        ...
        await conn.execute(
            """INSERT INTO drivers(user_id, phone, car_model, car_year, car_number, car_class, platform)
               VALUES($1,$2,$3,$4,$5,$6,'vk')
               ON CONFLICT (user_id) DO UPDATE SET
               phone=$2, car_model=$3, car_year=$4, car_number=$5, car_class=$6,
               docs_verified=0, platform='vk'""",
...
async def passenger_orders_list(message: Message):
    ...
    if not orders:
        await safe_send(uid, "У вас пока нет заказов.", keyboard=kb_main(vk_uid(message.from_id)))
        return
    for o in orders:
        text = fmt_order(o)
        kb = None
        ...
        elif o['status'] == 'completed' and not await has_rating(o['id'], vk_uid(message.from_id)):
            kb = Keyboard(inline=True)
            for i in range(1, 6):
...
async def driver_trips_list(message: Message):
    ...
    if await check_blacklist(vk_uid(message.from_id)):
        await safe_send(uid, "⛔ Вы заблокированы.")
        return
    if not await is_driver_registered(vk_uid(message.from_id)):
        await safe_send(uid, "Сначала зарегистрируйтесь как водитель.", keyboard=kb_main(vk_uid(message.from_id)))
        return
    async with _pg_pool.acquire() as conn:
        orders = await conn.fetch(
    ...
    if not orders:
        await safe_send(uid, "У вас пока нет поездок.", keyboard=kb_driver_menu(await has_active_sub(vk_uid(message.from_id))))
        return
    for o in orders:
        kb = None
        if o['status'] == 'taken':
...
async def show_tariffs(message: Message):
    ...
    for phone in DISPATCHER_PHONES:
        text += f"  {phone}\n"
    await safe_send(uid, text, keyboard=kb_main(vk_uid(message.from_id)))
...
async def subscription_handler(message: Message):
    ...
    if await check_blacklist(vk_uid(message.from_id)):
        await safe_send(uid, "⛔ Вы заблокированы.")
        return
    has_sub = await has_active_sub(vk_uid(message.from_id))
    # FIX: text всегда инициализируется
    if has_sub:
        async with _pg_pool.acquire() as conn:
            r = await conn.fetchrow("SELECT expires_date FROM subscriptions WHERE user_id=$1", vk_uid(message.from_id))
...
async def admin_panel(message: Message):
    uid = vk_uid(message.from_id)
    if vk_uid(message.from_id) not in ADMIN_IDS:
...
async def admin_orders(message: Message):
    if vk_uid(message.from_id) not in ADMIN_IDS:
        return
    await _send_admin_orders_page(vk_uid(message.from_id), 0)
...
async def admin_users(message: Message):
    if vk_uid(message.from_id) not in ADMIN_IDS:
        return
    await _send_admin_drivers_page(vk_uid(message.from_id), 0)
...
async def admin_verification(message: Message):
    ...
    if vk_uid(message.from_id) not in ADMIN_IDS:
        return
    async with _pg_pool.acquire() as conn:
        unverified = await conn.fetch("SELECT * FROM drivers WHERE docs_verified::int = 0 LIMIT 10")
    if not unverified:
...
async def admin_subscriptions(message: Message):
    ...
    async with _pg_pool.acquire() as conn:
        ...
        active = await conn.fetch(
            "SELECT * FROM subscriptions WHERE expires_date::date >= $1",
...
async def admin_stats(message: Message):
    ...
    async with _pg_pool.acquire() as conn:
        ...
        drv_stats = await conn.fetchrow(
            """SELECT
               COUNT(*) as total,
               COUNT(*) FILTER (WHERE docs_verified::int = 1) as verified
               FROM drivers"""
        )
        active_subs = await conn.fetchval(
            "SELECT COUNT(*) FROM subscriptions WHERE expires_date::date >= $1",
            datetime.now(TZ).date()
        )
...
async def unified_payload_handler(message: Message):
    try:
        ...
        if cmd == "verify_driver":
            ...
            async with _pg_pool.acquire() as conn:
                await conn.execute("UPDATE drivers SET docs_verified=1 WHERE user_id=$1", target_uid)
            ...
            if not target_uid:
                return await safe_send(uid, "❌ Ошибка: не найден ID водителя.")
            async with _pg_pool.acquire() as conn:
                await conn.execute("UPDATE drivers SET docs_verified=0 WHERE user_id=$1", target_uid)
        ...
        # --- Редактирование водителя администратором ---
            ...
            if field == "car_class":
                ...
                await bot.state_dispenser.set(vk_raw(uid), AdminEditStates.waiting_input)
        ...
        # --- Бан водителя ---
            ...
            async with _pg_pool.acquire() as conn:
                ...
                await conn.execute(
                    ...
                    target_uid, new_exp.strftime("%Y-%m-%d")
                )
                await conn.execute("DELETE FROM pending_subscriptions WHERE user_id=$1", target_uid)
                await conn.execute(
...
async def driver_profile(message: Message):
    ...
    if await check_blacklist(vk_uid(message.from_id)):
        return await safe_send(uid, "⛔ Вы заблокированы.")
    if not await is_driver_registered(vk_uid(message.from_id)):
        return await safe_send(uid, "Вы не зарегистрированы как водитель.", keyboard=kb_main(vk_uid(message.from_id)))
    async with _pg_pool.acquire() as conn:
        driver = await conn.fetchrow("SELECT * FROM drivers WHERE user_id=$1", vk_uid(message.from_id))
    if not driver:
        return await safe_send(uid, "Профиль не найден.", keyboard=kb_main(vk_uid(message.from_id)))
    has_sub = await has_active_sub(vk_uid(message.from_id))
    avg, cnt = await avg_rating(vk_uid(message.from_id))
...
async def trip_create_start(message: Message):
    ...
    await bot.state_dispenser.set(vk_raw(uid), TripStates.from_city)
    fsm_data[uid] = {}
    fsm_data_ts[uid] = asyncio.get_event_loop().time()
...
async def trip_from_city(message: Message):
    ...
    fsm_data[uid] = data
    fsm_data_ts[uid] = asyncio.get_event_loop().time()
    await bot.state_dispenser.set(vk_raw(uid), TripStates.to_city)
...
async def trip_to_city(message: Message):
    ...
    fsm_data[uid] = data
    fsm_data_ts[uid] = asyncio.get_event_loop().time()
    await bot.state_dispenser.set(vk_raw(uid), TripStates.trip_date)
...
async def trip_date_handler(message: Message):
    ...
    fsm_data[uid] = data
    fsm_data_ts[uid] = asyncio.get_event_loop().time()
    await bot.state_dispenser.set(vk_raw(uid), TripStates.trip_time)
...
async def trip_time_handler(message: Message):
    ...
    fsm_data[uid] = data
    fsm_data_ts[uid] = asyncio.get_event_loop().time()
    await bot.state_dispenser.set(vk_raw(uid), TripStates.seats_total)
...
async def trip_seats_handler(message: Message):
    ...
    fsm_data[uid] = data
    fsm_data_ts[uid] = asyncio.get_event_loop().time()
    await bot.state_dispenser.set(vk_raw(uid), TripStates.car_class)
...
async def trip_car_class_handler(message: Message):
    ...
    fsm_data[uid] = data
    fsm_data_ts[uid] = asyncio.get_event_loop().time()
    await bot.state_dispenser.set(vk_raw(uid), TripStates.price_per_seat)
...
async def search_trip_start(message: Message):
    ...
    if await check_blacklist(uid):
        return await safe_send(uid, "⛔ Вы заблокированы.")
    await bot.state_dispenser.set(vk_raw(uid), SearchTripStates.from_city)
    fsm_data[uid] = {}
    fsm_data_ts[uid] = asyncio.get_event_loop().time()
...
async def search_from_city(message: Message):
    ...
    fsm_data[uid] = data
    fsm_data_ts[uid] = asyncio.get_event_loop().time()
    await bot.state_dispenser.set(vk_raw(uid), SearchTripStates.to_city)
...
async def search_to_city(message: Message):
    ...
    fsm_data[uid] = data
    fsm_data_ts[uid] = asyncio.get_event_loop().time()
    await bot.state_dispenser.set(vk_raw(uid), SearchTripStates.trip_date)
...
# Сайт и Telegram-бот используют ключ "comfort+", здесь исторически "comfort_plus".
CAR_CLASS_ALIASES = {"comfort+": "comfort_plus", "comfort_plus": "comfort_plus"}
VALID_CAR_CLASSES = {"standard", "comfort", "comfort_plus", "comfort+", "minivan", "business"}


def normalize_car_class(cc: str) -> str:
    return CAR_CLASS_ALIASES.get((cc or "").strip(), (cc or "").strip())
...
async def create_site_order(req: SiteOrderRequest):
    ...
    req.car_class = normalize_car_class(req.car_class)
    ...
    async with _pg_pool.acquire() as conn:
        order = await conn.fetchrow(
            """INSERT INTO orders(
                passenger_id, from_city, to_city, trip_date, trip_time,
                passengers, car_class, wishes, distance_km, price, cancel_token, status
...
async def main():
    ...
    asyncio.create_task(announce_worker())
